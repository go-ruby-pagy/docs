...
`go-ruby-pagy/pagy` reimplements Ruby's `pagy` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-pagy/pagy` directly, with no Ruby runtime;
...
Because `pagy` is pure Go with cgo disabled, it also compiles to **WebAssembly**
